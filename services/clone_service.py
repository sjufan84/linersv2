...
async def get_clone_vocals(
        device, is_half, model_name, input_path, f0up_key, f0method, index_path,
        index_rate, filter_radius, resample_sr, rms_mix_rate, protect, opt_path
):
  config = Config()
...
async def clone_liner_vocals(input_audio: str, artist = "Joel Kaiser", f0_adjustment: int = 0):
    ...
    sr, audio = await get_clone_vocals(
        None, None, model_name, input_path, f0up_key, f0method, index_path,
        index_rate, filter_radius, resample_sr, rms_mix_rate, protect, opt_path
    )
